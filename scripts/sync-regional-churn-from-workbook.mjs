...
const STATUS_CHURN = "해지";
const TIMELINE_AUTHOR_NAME = "시스템";
const IMPORT_PREFIX = "[해지]";
...
function parseArgs(argv) {
  const options = {
    file: "",
    apply: false,
    backupDir: path.resolve(process.cwd(), "backups", "regional-churn-sync"),
    reportPrefix: "regional-churn-sync-report",
  };
...
function buildSourceRows(workbookPath) {
  ...
  const headers = Object.keys(rows[0]).slice(0, 22);
  const sourceRows = rows
    .slice(1)
    .map((row, index) => ({
      values: Object.values(row).slice(0, 22).map((value) => normalizeText(value)),
      index: index + 2,
    }))
    ...
    .map(({ values, index }) => ({
      ...
      contractStatus: values[4] || STATUS_CHURN,
...
async function loadCurrentChurnDeals(client) {
...
async function loadCurrentChurnTimelines(client) {
  const result = await client.query(`
    select
      dt.*
...
function buildNewDealRecord(source, productId) {
  return {
    id: randomUUID(),
    title: normalizeText(source.customerName) || normalizeText(source.companyName) || normalizeText(source.billingAccountNumber) || `해지-${source.sourceIndex}`,
    customer_id: null,
    value: 0,
    stage: "churned",
    probability: 0,
    expected_close_date: source.inboundDate || source.contractStartDate || source.contractEndDate || source.churnDate || null,
...
async function run() {
  ...
  const backupPath = path.resolve(options.backupDir, `regional-churn-before-sync-${getTimestampLabel(now)}.json`);
  ...
  try {
    const [currentDeals, currentTimelines, regionalProductMap] = await Promise.all([
      loadCurrentChurnDeals(client),
      loadCurrentChurnTimelines(client),
    ...
    const preview = {
      ...
      currentChurnCount: currentDeals.length,
    ...
    for (const source of creates) {
      ...
      const anchorDate = deal.churn_date || deal.contract_end_date || deal.contract_start_date || deal.inbound_date || now;
      await client.query(
        `
    ...
    const finalDeals = await loadCurrentChurnDeals(client);
    ...
    const report = {
      ...preview,
      backupPath,
      finalChurnCount: finalDeals.length,
